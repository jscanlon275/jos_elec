...
%% Figure 4: VERSION 2 correlation plots

figure('Renderer', 'painters', 'Position', [10 10 900 600]);
subplot(1, 3, 1);
%Shepherd's pi
[stats] = ScatterOutliers_2(Accdata_analyze(:,3), Accdata_analyze(:,4), 1000, false, '-winter',[200 600],[350 600]);
title({'Walking Acc Variance'; '(mag)'  }, 'FontSize', 15)
set(gca, 'LineWidth', 2)
xlabel('Active')
ylabel('Passive')
...
title({'Standing P3 Amplitude'; '(\muV)'}, 'FontSize', 15)
set(gca, 'LineWidth', 2)
xlabel('Active')
ylabel('Passive')

subplot(1, 3, 3);
[stats] = ScatterOutliers_2(P3_analyze(:, 3),P3_analyze(:,4), 1000, false, '-winter', [-5 20],[-1 20]);
title({'Walking P3 Amplitude'; '(\muV)'}, 'FontSize', 15)
set(gca, 'LineWidth', 2)
xlabel('Active')
ylabel('Passive')
...
%% Figure 4: VERSION 3 correlation plots
...
%% SNR Calculation (non-chronological)
% electrode = find(strcmp({EEG.chanlocs.labels}, 'Pz'));
        ...
        for i_type = 1:2;
            for i_cond = 1:3;
                ...
                for i_trials = 1:100
                    ...
                    SNR_nonchron(i_sub,i_type, i_cond, i_trials, i_wt) = nanmean(signal)/nanmean(noise);
                    
                    if i_trials > 1;
                        if isnan(SNR_nonchron(i_sub,i_type, i_cond, i_trials, i_wt));
                            if ~isnan(SNR_nonchron(i_sub,i_type, i_cond, i_trials-1, i_wt));
                                SNR_end(i_sub, i_type, i_cond, i_wt) = SNR_nonchron(i_sub,i_type, i_cond, i_trials-1, i_wt);
                            end
                        end
                    end
...
%% SNR Calculation (non chron)
% electrode = find(strcmp({EEG.chanlocs.labels}, 'Pz'));
SNR_nonchron= nan(18,2, 3,100,2);

%ERP_calc = ERP_st1(i_wt).ERP_singletrial1(:,1, P3TWINDOW_Gave,:,:, 1, 1:100)
for i_wt = 1:length(trialrej)
    for i_sub = 1:length(SUBJ)
        ...
        for i_type = 1:2;
            for i_cond = 1:3;
             for i_perm  = 1:50;
                 ...
                 ERP_calc = ERP_st1(i_wt).ERP_singletrial1(i_sub,1, 1,i_cond,i_type, 1, 1:100);
             ...
             shuf_nonnan = shuffle(nonnan);
                ...
                ERP_st1(i_wt).ERP_singletrial1(i_sub,1, P3TWINDOW_Gave,i_cond,i_type, i_stim, :)= ERP_st1(i_wt).ERP_singletrial1(i_sub,1, P3TWINDOW_Gave,i_cond,i_type, i_stim, neworder);
                curr_trials = 1;
                for i_trials = 1:100
                    ERP_odd =  squeeze(nanmean(ERP_st1(i_wt).ERP_singletrial1(i_sub,1, P3TWINDOW_Gave,i_cond,i_type, i_stim, 1:2:curr_trials), 7));
                    ERP_even = squeeze(nanmean(ERP_st1(i_wt).ERP_singletrial1(i_sub,1, P3TWINDOW_Gave,i_cond,i_type, i_stim, 2:2:curr_trials), 7));
                    ...
                    if i_trials > 1;
                        if isnan(SNR_nonchron(i_sub,i_type, i_cond, i_trials, i_wt));
                            if ~isnan(SNR_nonchron(i_sub,i_type, i_cond, i_trials-1, i_wt));
                                SNR_end(i_sub, i_type, i_cond, i_perm, i_wt) = SNR_nonchron(i_sub, i_type, i_cond, i_perm, i_trials-1, i_wt);
                            end
                        end
                    end
                    curr_trials = curr_trials+1;
...
% Figure 5 (version 2): SNR comparison graph
% WALKCONDS = {'standing', 'walkingA', 'walkingT'};
% ETYPE = {'Active', 'Passive'};
mymap = [ 0.1328    0.5430    0.1328;  0.5977    0.1953    0.7969];
f8  = figure('Renderer', 'painters', 'Position', [10 10 800 600]);
...
for i_type = 1:2;
    ...
    subplot(2,2,i_type)
    SNR_means1 =    squeeze(mean(nanmean(SNR_nonchron(:,i_type, i_cond,:, :,1),4),1));
    SNR_std1 =      squeeze(std(nanmean(SNR_nonchron(:,i_type, i_cond,:, :, 1),4)))./sqrt(length(SUBJ));
    SNR_means2 =    squeeze(mean(nanmean(SNR_nonchron(:,i_type, i_cond,:, :,2),4),1));
    SNR_std2 =      squeeze(std(nanmean(SNR_nonchron(:,i_type, i_cond, :,:, 2),4)))./sqrt(length(SUBJ));
    b=   boundedline( [1:100] , SNR_means2, SNR_std2, 'k',[1:100] , SNR_means1, SNR_std1, colour,  'nan', 'gap', 'alpha','transparency', 0.4 );
...
disp([' yfit = ', num2str(p(1)),'x + ', num2str(p(2))])
 ...
 % This demonstrates how much of the equation yfit = p(1) * x[SNR_means...]
 % + p(2) predicts y (percentage)
%% Possible supplementary files: Data noise raincloud plots
colours = [8 10 7 11];
cl = [[1 0 0]; [0 0 1];[1 0 1] ;[0 1 1]]
...
for i_cond = 1:2
    for i_type = 1:2
        tempdat{i_cond, i_type} = STDnoise_analyze(:,((2*i_cond-2)+i_type))
    end
end
...
% save
thetitle = ['Data noise and SNR distributions' ]
suptitle({thetitle });
print([PATHOUT, thetitle], '-dpng')