...
for i=1:length(allACC)
    if strcmpi(allACC{1,i}.setname,'Accelerometer left') %change: removed 'resampled' from setname
        AccL=allACC{1,i};
        AccLOrder=i;
    elseif strcmpi(allACC{1,i}.setname,'Accelerometer right')%change: removed 'resampled' from setname
        AccR=allACC{1,i};
        AccROrder=i;
    elseif strcmpi(allACC{1,i}.setname,'EEG')
        EEG=allACC{1,i};
        EEGOrder=i;
    end  
end
...
Path=mfilename('fullpath'); Path=Path(1:end-11);
