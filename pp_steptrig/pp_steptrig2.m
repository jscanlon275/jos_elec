function [EEG, allACC] = pp_steptrig(allACC, varargin)
...
for i=1:length(allACC)
    if strcmpi(allACC{1,i}.setname,'Acc1 resampled') %change: removed 'resampled' from setname
        AccL=allACC{1,i};
        AccLOrder=i;
    elseif strcmpi(allACC{1,i}.setname,'Acc2 resampled')%change: removed 'resampled' from setname
        AccR=allACC{1,i};
        AccROrder=i;
    elseif strcmpi(allACC{1,i}.setname,'EEG resampled')
        EEG=allACC{1,i};
        EEGOrder=i;
    end  
end
...
if UseTemp
    load([Path 'DefStepTemp.mat']);

    if (Step.Fs~=AccL.srate) || (Step.Fs~=AccR.srate)
        error('Sampling frequency of the loaded template does not match sampling frequency of the accelerometer signal.')
    end
    
    figure(213)
    subplot(1,4,1:3)
        plot(AccL.times,AccL.data(2,:)); hold on;
        plot(AccL.times,AccR.data(2,:));
        legend('Exp','Par')
        title('Zoom in and compare AccX and template; to continue press enter in command window.');
    subplot(1,4,4)
        plot(Step.Time, Step.Tmp); hold on;
...
if ~UseTemp
    ...
    while startTemplate
        ...
        Step.Tmp=AccL.data(2,x1:x2);
        Step.Time=AccL.times(x1:x2)-AccL.times(x1);
        Step.Fs=AccL.srate;
...
for i=1:length(AccL.data(2,:))-Wdw
    AccL.ccorr(i) = xcorr(AccL.data(2,i:i+Wdw), Step.Tmp,0);
    AccR.ccorr(i) = xcorr(AccR.data(2,i:i+Wdw), Step.Tmp,0);
end
...
for n = length(EEG.event)+1:length(EEG.event)+num
    count=count+1;
    EEG.event(n).type = 'parstep';
    EEG.event(n).latency = AccR.peakSmp(count);
    EEG.event(n).urevent = n;
end


allACC{1,EEGOrder}=EEG;
AccL.event=EEG.event;
allACC{1,AccLOrder}=AccL;
AccR.event=EEG.event;
allACC{1,AccROrder}=AccR;
