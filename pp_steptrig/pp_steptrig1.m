...
for i=1:length(allACC)
    if strcmpi(allACC.setname,'Accelerometer left') %change: removed 'resampled' from setname
        AccL=allACC{1,i};
        AccLOrder=i;
    elseif strcmpi(allACC.setname,'Accelerometer right')%change: removed 'resampled' from setname
        AccR=allACC{1,i};
        AccROrder=i;
    elseif strcmpi(allACC.setname,'EEG')
        EEG=allACC{1,i};
        EEGOrder=i;
    end  
end
